const SITE = {
  businessName: "Sell Mineral Rights Michigan",
  businessEmail: "michigan@sellmineralrightsmichigan.com",
  siteUrl: "https://sellmineralrightsmichigan.com"
};
...
function emailFallback(res, lead) {
  ...
  return redirect(res, `mailto:${SITE.businessEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(message)}`);
}
...
function leadFrom(body, req) {
  return {
    name: first(body, ["name", "contact_name", "owner_name"]),
    email: first(body, ["email", "emailAddress"]),
    phone: first(body, ["phone", "phoneNumber"]),
    message: first(body, ["message", "propertyDetails", "property_details"]),
...
function invalid(lead) {
  if (!lead.name || !lead.email || !lead.phone || !lead.message) return "Please complete each required field.";
...
function solicitation(lead) {
  ...
  const pitches = ["guest post", "link building", "backlinks", "domain authority", "seo services", "web design services", "first page of google", "marketing agency", "crypto investment"];
  return Boolean(lead.honeypot) || pitches.some((term) => text.includes(term)) || (text.match(/https?:\/\//g) || []).length > 2;
}
...
async function sendEmail(to, lead) {
  ...
  const rows = [["Name", lead.name], ["Email", lead.email], ["Phone", lead.phone], ["Message", lead.message], ["Source", lead.source]];
  const text = rows.map(([label, value]) => `${label}: ${value}`).join("\n");
  const html = rows.map(([label, value]) => `<p><strong>${escapeHtml(label)}:</strong> ${escapeHtml(value)}</p>`).join("");
  const response = await fetch("https://api.sendgrid.com/v3/mail/send", {
    ...
    body: JSON.stringify({
      ...
      categories: ["mineral-rights-lead", "sellmineralrightsmichigan-com"]
    })
  });
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!solicitation(lead)) {
      ...
      const recipients = clean(process.env.CONTACT_NOTIFICATION_RECIPIENTS || process.env.RANKHOUND_NOTIFICATION_EMAIL || SITE.businessEmail)
    ...
    console.error(error);
    return json(res, 500, { ok: false, error: "The form could not be submitted. Please call or email the Michigan owner desk." });
  }
};
